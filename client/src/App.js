...
function App() {
	...
	const GetTodos=()=>{
		fetch(API_BASE+"/todos").then(res=>res.json()).then(data=>setTodos(data))
	}

	const completeTodo = async id=>{
		const data= await fetch(`${API_BASE}/todo/complete/${id}`,{method:"put"}).then(res=>res.json());
		setTodos(todos=>todos.map(todo=>{
			if(todo!==null&&data!==null&& (todo._id===data._id))
			{
	...
	const deleteTodo= async id=>{
		...
		setTodos(todos=>todos.filter(todo=>todo!==null&&data!==null&& todo._id!==data._id));
